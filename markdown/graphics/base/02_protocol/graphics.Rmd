...
sec3 <- 0
```
### `r sec1`.`r sec2`. Taking control of scaling a single graph

Scaling of the device canvas in base R is defined in the context of a rectangular *plot region*, where this region defines the area within which data are conventionally plotted (though the scale extends to the whole canvas and data can be plotted anywhere). The scaling function ` plot.window() ` establishes a coordinate system based on the margins between this region and the edge of the canvas, the limits of the x and y coordinates that define this region, and parameters that control buffering the limits of the coordinates from the edges of the graph. 
...
Multi-panel plots will be covered in more detail later, so we will not experiment with outer margins here. Just note for now that the default values here are typically zero and thus have no effect on scaling. The space available between the plot region and the edge of the canvas will be defined entirely be the inner margins.
...
sec3 <- sec3 + 1
```
#### `r sec1`.`r sec2`.`r sec3` Inverted and log scales
...
- ` plot() ` : A high level function for creating a new plot with a single line of code. This function is an S3 abstraction, which in this case results in a call to ` plot.default() `.
    - ` ylim `: The limits of the values defining the plot region on the y axis. Note that this argument is passed to ` plot() ` generically, and its effect on scaling is not realized until ` plot.default() ` ultimately passes the value assigned to ` ylim ` on to ` plot.window() `.
